...
-- They were defined in drizzle/schema.ts and captured in the drizzle
-- snapshots, but no CREATE TABLE migration was ever committed for them —
-- they only existed on databases provisioned via `drizzle-kit push`.
-- On any database migrated through the runtime file migrator (the startup
-- auto-migration in server/_core/index.ts), the tables were missing, so
-- startOrchestrator()'s existence probe failed and the "Start Autonomous
-- Agent" button silently did nothing.
--
-- All statements are idempotent (CREATE TABLE IF NOT EXISTS) so this is safe
-- to apply on databases that already have the tables from an earlier push.
