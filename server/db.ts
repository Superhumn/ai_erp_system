...
export async function getShipmentByPurchaseOrderId(purchaseOrderId: number) {
  const db = await getDb();
  if (!db) return undefined;
...
export async function receivePurchaseOrderItems(
  ...
  for (const item of items) {
    ...
    if (item.rawMaterialId) {
      ...
      // Update purchaseOrderRawMaterials link status
      const rmLink = await db.select().from(purchaseOrderRawMaterials)
        .where(and(
          eq(purchaseOrderRawMaterials.purchaseOrderItemId, item.purchaseOrderItemId),
          eq(purchaseOrderRawMaterials.rawMaterialId, item.rawMaterialId)
        ))
        .limit(1);

      if (rmLink[0]) {
        const prevRmReceived = parseFloat(rmLink[0].receivedQuantity?.toString() || '0');
        const orderedQty = parseFloat(rmLink[0].orderedQuantity?.toString() || '0');
        const newRmReceived = prevRmReceived + item.quantity;
        const newStatus = newRmReceived >= orderedQty ? 'received' : 'partial';

        await db.update(purchaseOrderRawMaterials)
          .set({
            receivedQuantity: newRmReceived.toFixed(4),
            status: newStatus,
          })
          .where(eq(purchaseOrderRawMaterials.id, rmLink[0].id));
      }
    }
...
// Generate low stock alerts and auto-create suggested POs
export async function generateLowStockAlerts() {
  ...
  const suggestedPOsCreated: number[] = [];
  ...
  for (const material of lowStockMaterials) {
    ...
    if (!existing[0]) {
      ...
      const { id: alertId } = await createAlert({
        type: 'low_stock',
        severity: parseFloat(material.quantity?.toString() || '0') === 0 ? 'critical' : 'warning',
      ...
      // Auto-create suggested PO if vendor is configured
      if (rawMat) {
        const vendor = await getPreferredVendorForMaterial(material.rawMaterialId);
        if (vendor) {
          // Check if suggested PO already exists for this material
          const existingSuggested = await db.select().from(suggestedPurchaseOrders)
            .where(and(
              eq(suggestedPurchaseOrders.vendorId, vendor.id),
              eq(suggestedPurchaseOrders.status, 'pending')
            ))
            .limit(1);

          // Calculate suggested order quantity (reorder quantity or difference to reorder point * 1.5)
          const currentQty = parseFloat(material.quantity?.toString() || '0');
          const reorderPoint = parseFloat(material.reorderPoint?.toString() || '0');
          const reorderQty = parseFloat(material.reorderQuantity?.toString() || '0');
          const suggestedQty = reorderQty > 0 ? reorderQty : Math.ceil((reorderPoint - currentQty) * 1.5);

          if (!existingSuggested[0]) {
            // Create new suggested PO
            const suggestedPo = await createSuggestedPurchaseOrder({
              vendorId: vendor.id,
              priority: currentQty === 0 ? 'critical' : 'high',
              status: 'pending',
              estimatedTotal: '0', // Will be updated when items are added
              aiRationale: `Auto-generated due to low stock alert. ${rawMat.name} is at ${currentQty} ${material.unit || 'units'}, below reorder point of ${reorderPoint}.`,
            });

            // Add item to suggested PO
            await createSuggestedPoItem({
              suggestedPoId: suggestedPo.id,
              rawMaterialId: material.rawMaterialId,
              productId: null,
              suggestedQuantity: suggestedQty.toString(),
              unit: material.unit || 'EA',
              estimatedUnitPrice: rawMat.unitCost || '0',
              totalAmount: (suggestedQty * parseFloat(rawMat.unitCost?.toString() || '0')).toFixed(2),
              reason: `Low stock: current ${currentQty}, reorder point ${reorderPoint}`,
            });

            suggestedPOsCreated.push(suggestedPo.id);

            // Link alert to suggested PO
            await db.update(alerts).set({
              linkedRecommendationId: suggestedPo.id,
            }).where(eq(alerts.id, alertId));
          } else {
            // Add item to existing suggested PO if not already there
            const existingItems = await getSuggestedPoItems(existingSuggested[0].id);
            const hasItem = existingItems.some(item => item.rawMaterialId === material.rawMaterialId);

            if (!hasItem) {
              await createSuggestedPoItem({
                suggestedPoId: existingSuggested[0].id,
                rawMaterialId: material.rawMaterialId,
                productId: null,
                suggestedQuantity: suggestedQty.toString(),
                unit: material.unit || 'EA',
            ...
            // Link alert to existing suggested PO
            await db.update(alerts).set({
              linkedRecommendationId: existingSuggested[0].id,
            }).where(eq(alerts.id, alertId));
          }
        }
      }
  ...
  return { alerts: createdAlerts, suggestedPOs: suggestedPOsCreated };
}
