...
/**
 * Look up a project by its exact name. The column collation is
 * case-insensitive, so "Website Redesign" and "website redesign" are the same
 * project — which is what an imported spreadsheet expects.
 */
export async function findProjectByName(name: string, companyId?: number) {
  const db = await getDb();
  if (!db) return undefined;
  const trimmed = name.trim();
  if (!trimmed) return undefined;
  const conditions = [eq(projects.name, trimmed)];
...
/**
 * Attach to the project with this name, or open it. Used by the spreadsheet
 * importers so re-running the same sheet updates nothing and duplicates
 * nothing — the existing project is reused as-is.
 */
export async function findOrCreateProjectByName(
  name: string,
  data: Omit<InsertProject, "name">,
): Promise<{ id: number; created: boolean }> {
  const existing = await findProjectByName(name.trim(), data.companyId ?? undefined);
