...
async function startServer() {
  ...
  app.get('/api/shopify/callback', oauthCallbackLimiter, async (req, res) => {
    ...
    try {
      ...
      await dbModule.upsertShopifyStore(shopDomain, { companyId: user.companyId || undefined, storeDomain: shopDomain, storeName: shopInfo.shop.name || shopDomain, accessToken: encryptedToken, apiVersion: '2024-01', isEnabled: true, syncInventory: true, syncOrders: true, inventoryAuthority: 'hybrid' });
      await dbModule.createSyncLog({ integration: 'shopify', action: 'store_connected', status: 'success', details: `Connected store: ${shopInfo.shop.name} (${shopDomain})` });
