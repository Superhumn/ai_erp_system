  ...
  const importMutation = trpc.recruiting.importFromLinkedIn.useMutation({
    onSuccess: data => {
      setFormData(prev => ({
        ...
        source: "linkedin",
        notes:
          prev.notes || (data.location ? `Location: ${data.location}` : ""),
      }));
      if (data.note) toast.warning(data.note);
      else toast.success("Pulled candidate info from LinkedIn");
    },
    onError: err => {
      toast.error(err.message || "Could not import from LinkedIn");
    },
  });
...
Provide: 1) Score X/10, 2) Key strengths, 3) Concerns, 4) Recommendation (advance/hold/reject). Be concise.`,
  ...
  const stats = {
    ...
    pipeline: candidates.filter(c => !["hired", "rejected"].includes(c.stage))
      .length,
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-4 text-xs flex-wrap">
          ...
          <div>
            ...
            <span className="font-bold">{stats.pipeline}</span>
          </div>
          <div className="h-4 w-px bg-border" />
          <div>
          ...
          <div>
            ...
            <span className="font-bold text-green-600">{stats.offers}</span>
          </div>
          <div className="h-4 w-px bg-border" />
          <div>
            ...
            <div className="space-y-3">
              <div className="space-y-1 pb-3 border-b">
                <Label className="text-xs flex items-center gap-1">
                  <Linkedin className="h-3.5 w-3.5 text-[#0a66c2]" /> Import
                  from LinkedIn
                </Label>
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    disabled={!linkedinUrl.trim() || importMutation.isPending}
                  >
                ...
                <p className="text-[10px] text-muted-foreground">
                  Paste a public profile URL to auto-fill the fields below.
                </p>
              </div>
              ...
              <div className="space-y-1">
                ...
                <Textarea
                  placeholder="Paste resume text or LinkedIn summary..."
                      ...
                      <div className="flex items-center gap-3">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            <Badge className={stageColors[c.stage]}>
                              {c.stage}
                            </Badge>
                            {c.score !== null && (
                              <Badge
                          ...
                          <div className="flex items-center gap-3 mt-0.5 text-muted-foreground">
                            <span>{c.source}</span>
                            <span>
                              {format(new Date(c.appliedAt), "MMM d")}
                            </span>
