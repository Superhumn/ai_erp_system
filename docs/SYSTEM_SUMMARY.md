...
- **131** top-level tRPC routers · **1,263** procedures (`scripts/dump-trpc-paths.ts`); client
  reach last audited at 81% — rerun `pnpm audit:coverage` for a current figure
- Two legacy monoliths still live: `server/db.ts` (**~17.7k lines**),
  `drizzle/schema.ts` (**~8k lines**), ~295 tables. `server/routers.ts` was split
  one-file-per-key into `server/routers/` (parity-verified against the list above).
- **76** unit test files / **1,185** tests — **all passing** ✅
- e2e: a single Playwright `smoke.spec.ts`

> Note: line counts drift. `DB_INDEX.md` is regenerated by `pnpm index:legacy`.
> There is no equivalent generated document for the router tree: `server/routers/index.ts`
> is the live entry point, and the one-file-per-key directory listing is the map.
...
- **`.strict-baseline.json` tracks the accepted strict-null errors** (see the
  file for the current count — single digits). Grandfathered, not yet burned down.
...
`server/db/` looks like a finished per-feature refactor but is **unwired** —
most callers still `import * as db from "./db"` (the file). **Adding a helper
only to `server/db/<feature>.ts` is invisible to those callers.** Until it's
wired, new DB helpers go in `server/db.ts` (per `CLAUDE.md`); `DB_INDEX.md`
lists the legacy-only exports that quantify the gap.

`server/routers/` no longer has this problem: the monolith was split into it
one-file-per-key and `server/_core/index.ts` imports `server/routers/index.ts`.
`server/routers/_shared.ts` holds the helpers the routers still share and
should be dissolved into service modules over time.
